...
-(void) applicationDidFinishLaunching: (NSNotification*)notification
{
    NS_DURING
		int		x = 0,
				count = [[UKITunesLibrary defaultLibrary] playlistCount];
		
		//NSLog( @"hierarchy=%@", [[UKITunesLibrary defaultLibrary] playlistHierarchy] );
		...
		for( x = 0; x < count; x++ )
        {
			...
			NSLog(@"Playlist: %@", [currPL name]);
            for( trX = 0; trX < trCount; trX++ )
            {
				trItem = [currPL trackDictionaryForItemAtIndex: trX];
                if( trItem )
                    [tracks addObject: trItem];
            }
...
-(void) markExistingFiles: (id)sender
{
    ...
    {
        [self markExistingFilesInLibrary];
...
// Copy/delete items (i.e. sync library checkmarks and MP3 player):
-(void) markChangedFilesInLibraries
{
	int						count = [[UKITunesLibrary defaultLibrary] playlistCount],
							plX = 0;
    ...
    for( plX = 0; plX < count; plX++ )
    {
        UKITunesPlaylist* currPlist = [[UKITunesLibrary defaultLibrary] playlistAtIndex: plX];
		currPlistName = [currPlist name];
        ...
        {
            int		trackCount = [currPlist count],
					trX = 0;
			...
			for( trX = 0; trX < trackCount; trX++ )
            {
				currTrack = [currPlist trackDictionaryForItemAtIndex: trX];
            ...
            NSEnumerator*	trackEnny = [currPlistLastTimeSyncedTrackIDs objectEnumerator];
...
//      Loop over all tracks in the library and determine
//      whether files for them exist on the MP3 player.
// -----------------------------------------------------------------------------

-(void) markExistingFilesInLibrary
{
    UKITunesPlaylist*	plist = [[UKITunesLibrary defaultLibrary] mainLibraryPlaylist];
    NSDictionary*		track = nil;
    int					x = 0,
						count = [plist count];
    
    // Set up & show progress bar:
    [status setStringValue: [NSString stringWithFormat: @"Checking files in \"%@\"", [plist name]]];
    [progress setMaxValue: count];
    [progress setDoubleValue: 0];
    [progress setHidden: NO];
    [progress display];
    ...
    for( x = 0; x < count; x++ )
    {
		track = [plist trackDictionaryForItemAtIndex: x];
...
- (int)numberOfRowsInTableView:(NSTableView *)tableView
{
    if( tableView == tracksView )
    {
        ...
        if( selRow > -1 )
        {
            NSDictionary*		outlineItem = [playlistsView itemAtRow: selRow];
            UKITunesPlaylist*	plist = [[UKITunesLibrary defaultLibrary] playlistWithID: [outlineItem objectForKey: @"persistentID"]];
...
- (id)tableView:(NSTableView *)tableView objectValueForTableColumn:(NSTableColumn *)tableColumn row:(int)row
{
    ...
    if( tableView == tracksView )
    {
        ...
        if( selRow > -1 )
        {
            NSDictionary*		outlineItem = [playlistsView itemAtRow: selRow];
            UKITunesPlaylist*	plist = [[UKITunesLibrary defaultLibrary] playlistWithID: [outlineItem objectForKey: @"persistentID"]];
            NSDictionary*		currTrack = [plist trackDictionaryForItemAtIndex: row];
			if( !currTrack )
				return @"- unknown -";
			
            if( [ident isEqualToString: @"Location"] )
            {
                NSString*       path = [[NSURL URLWithString: [currTrack objectForKey: ident]] path];
                ...
                return [currTrack objectForKey: ident];
        }
        else
            return @"";
    }
    ...
    if( tableView == tracksView )
    {
        ...
        if( selRow > -1 )
        {
			...
			if( !currTrack )
				return;
            NSString*			ident = [tableColumn identifier];
            NSString*			path = [[NSURL URLWithString: [currTrack objectForKey: @"Location"]] path];
            NSArray*			allowedExts = [[self playerSettings] objectForKey: @"SupportedExtensions"];
...
- (id)outlineView:(NSOutlineView *)outlineView child:(int)index ofItem:(id)item
{
	if( item == nil )
	{
		NSArray*	rootLevel = [[UKITunesLibrary defaultLibrary] playlistHierarchy];
		return [rootLevel objectAtIndex: index];
	}
	else
		return [[item objectForKey: @"subItems"] objectAtIndex: index];
}
...
- (BOOL)outlineView:(NSOutlineView *)outlineView isItemExpandable:(id)item
{
	if( item == nil )
		return YES;
	return( [[item objectForKey: @"subItems"] count] > 0 );
}
...
- (int)outlineView:(NSOutlineView *)outlineView numberOfChildrenOfItem:(id)item
{
	...
	{
		NSArray*	rootLevel = [[UKITunesLibrary defaultLibrary] playlistHierarchy];
		return [rootLevel count];
	}
	else
		return [[item objectForKey: @"subItems"] count];
}

- (id)outlineView:(NSOutlineView *)outlineView objectValueForTableColumn:(NSTableColumn *)tableColumn byItem:(id)item
{
	UKITunesPlaylist*	plist = [[UKITunesLibrary defaultLibrary] playlistWithID: [item objectForKey: @"persistentID"]];
	
	if( [[tableColumn identifier] isEqualToString: @"copy"] )
		return [NSNumber numberWithBool: NO];
	else
		return [plist valueForKey: [tableColumn identifier]];
}
...
- (void)outlineViewSelectionDidChange:(NSNotification *)notification
{
	...
	if( selRow >= 0 )
	{
		NSDictionary*		item = [playlistsView itemAtRow: selRow];
		UKITunesPlaylist*	plist = [[UKITunesLibrary defaultLibrary] playlistWithID: [item objectForKey: @"persistentID"]];
	}
}
